...
class InputTests(unittest.TestCase):
    ...
    def test_process_as_form(self, job_number, dcn_key, was_prev_matched,
        ...
        fake_dilfo_insert = """
            INSERT INTO company_projects (job_number, receiver_email, closed)
            VALUES (?, 'alex.roy616@gmail.com', ?)
        """
        fake_match_insert = """
            INSERT INTO attempted_matches (job_number, verifier, ground_truth)
            VALUES (?, 'alex.roy616@gmail.com', ?)
        """
    ...
    def test_process_as_reply(self, job_number, dcn_key, ground_truth,
        ...
        # set-up new entries in db, if necessary
        fake_dilfo_insert = """
            INSERT INTO company_projects (job_number, closed)
            VALUES (?, ?)
        """
        fake_match_insert = """
            INSERT INTO attempted_matches (job_number, ground_truth)
            VALUES (?, ?)
        """
        with create_connection() as conn:
            conn.cursor().execute(fake_dilfo_insert, [job_number, was_prev_closed])
        ...
        with create_connection() as conn:
            company_projects_post = pd.read_sql("SELECT * FROM company_projects WHERE job_number=?", conn, params=[job_number])
            attempted_matches_post = pd.read_sql("SELECT * FROM attempted_matches WHERE job_number=?", conn, params=[job_number])
        self.assertEqual(len(company_projects_pre), len(company_projects_post))
        self.assertEqual(company_projects_post.iloc[0].closed, was_prev_closed or ground_truth)
        self.assertEqual(any(attempted_matches_post.ground_truth), was_prev_closed or ground_truth)
        self.assertEqual(len(attempted_matches_pre) + (not was_prev_closed), len(attempted_matches_post))
        self.assertEqual(list(attempted_matches_pre.columns), list(attempted_matches_post.columns))
        self.assertEqual(list(company_projects_pre.columns), list(company_projects_post.columns))
