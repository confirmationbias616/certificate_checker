...
def communicate(web_df, dilfo_row, test=False):
	...
	def send_match():
		...
		feedback_msg = (
			...
			f"\t{link_constructor.format(dilfo_row.job_number, 0, web_df.dcn_key, receiver_email)}\n"
			f"\tlink is accurate match for my project\n"
			f"\t{link_constructor.format(dilfo_row.job_number, 1, web_df.dcn_key, receiver_email)}\n"
			f"\tlink is close but seems to relate to a different phase or "
			f"stage.\n"
			f"\t{link_constructor.format(dilfo_row.job_number, 2, web_df.dcn_key, receiver_email)}\n"
			f"\n"
